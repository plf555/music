...
public class SongController {
     ...
     * 根据用户id和歌曲id创建下载记录
     */
    @GetMapping("/createDownloadHistory")
    public Map createDownloadHistory(Integer userId, Integer songId){

        return songService.createDownloadHistory(userId, songId);
     ...
     * 根据userId查询用户的播放记录
     */
    @GetMapping("/getDownloadHistoryByUserId")
    public Map getDownloadHistoryByUserId(Integer userId){
        return songService.getHistoryByUserId(userId);
    }
    ...
    public Map getRecommendSong(Integer userId){
        if(userId != null && !userId.equals("")){
            ...
            return songService.getRecommendSongWithNoId();
        }
    }
